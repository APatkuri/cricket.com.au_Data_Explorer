...
if(format_type and comp_name and match_name):
    ...
    if os.path.exists(file_name):
        ...
        if(len(match_bowling_df) > 0):
            ...
            bowl_team_stats = match_bowling_df.groupby(["inningNumber", "bowlingTeamName"]).agg(
            ...
            bowler_stats = match_bowling_df.groupby(["bowlerPlayerName", "bowlingTeamName"]).agg(
                TotalDeliveries=('ballNumber', 'count'),  # Total balls bowled
                RunsConceded=('runsConceded', 'sum'),  # Total Runs Given
                Wickets=('isWicket', lambda x: x.sum()),  # Count of non-null wickets
                FalseShots=('battingConnectionId', lambda x: x.isin(false_shot_list).sum())  # Count False Shots
            ...
            bowler_stats = bowler_stats.sort_values(by="bowlingTeamName", ascending=False)

            st.dataframe(bowl_team_stats.style.highlight_max(color='green', axis=0, subset=['FalseShots', 'FalseShot%'])
                         .highlight_min(color='green', axis=0, subset=['FalseShotPerDismissial', 'BallsPerFalseShot', 'RunsPerFalseShot'])
                        .format({'FalseShotPerDismissial': '{:.2f}', 'BallsPerFalseShot': '{:.2f}', 'RunsPerFalseShot': '{:.2f}', 'RunsConceded': '{:.2f}', 'FalseShot%': '{:.2f}'}))
